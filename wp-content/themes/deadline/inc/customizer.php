...
// Extra styles
function deadline_customizer_stylesheet() {
	
	// Stylesheet
	wp_enqueue_style( 'deadline-customizer-css', get_template_directory_uri().'/inc/customizer-styles.css', NULL, NULL, 'all' );
...
function deadline_customize_register( $wp_customize ) {
	...
	// Custom help section
	class Deadline_WP_Help_Customize_Control extends WP_Customize_Control {
		public $type = 'text_help';
		public function render_content() {
			$deadline_ep_activated = '';
			if ( get_option( 'deadline_ep_license_status' ) == 'valid' ) {
				$deadline_ep_activated = 'bnt-customizer-ep-active';
			}
			echo '
				<div class="bnt-customizer-help">
					<a class="bnt-customizer-link bnt-support-link" href="http://www.ilovewp.com/documentation/deadline/" target="_blank">
						<span class="dashicons dashicons-book">
						</span>
						'.esc_html__( 'Theme Documentation', 'deadline' ).'
					</a>
					<a class="bnt-customizer-link bnt-support-link" href="https://wordpress.org/support/theme/deadline/" target="_blank">
						<span class="dashicons dashicons-sos">
						</span>
						'.esc_html__( 'Support Forum', 'deadline' ).'
					</a>
					<a class="bnt-customizer-link bnt-rate-link" href="https://wordpress.org/support/theme/deadline/reviews/" target="_blank">
						<span class="dashicons dashicons-heart">
						</span>
						'.esc_html__( 'Rate Deadline (thanks!)', 'deadline' ).'
					</a>
					<a class="bnt-customizer-link bnt-ep-link" href="http://eepurl.com/cvTvUL" target="_blank">
						...
						'.esc_html__( 'Subscribe to our Newsletter', 'deadline' ).'
					</a>
				</div>
			';
		}
	...
	$wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';
		...
		$wp_customize->add_section( 
			'deadline_theme_support', 
			array(
				'title' => esc_html__( 'Theme Help & Support', 'deadline' ),
				'priority' => 19,
			) 
		);
		...
		$wp_customize->add_control(
			new Deadline_WP_Help_Customize_Control(
			$wp_customize,
			'deadline_support', 
				array(
					'section' => 'deadline_theme_support',
					'type' => 'text_help',
				)
			)
		);
	...
	$wp_customize->add_section( 'deadline_header_options', array(
		'title'		  => esc_html__( 'Header', 'deadline' ),
		'panel'		  => 'deadline_panel',
	) );

		// Featured Categories checkbox
		$wp_customize->add_setting( 'deadline_header_display_tagline', array(
			'default'           => 1,
			'sanitize_callback' => 'deadline_sanitize_checkbox',
		) );

		$wp_customize->add_control( 'deadline_header_display_tagline', array(
			'label'             => esc_html__( 'Display site title and tagline in the header.', 'deadline' ),
			'section'           => 'deadline_header_options',
			'type'              => 'checkbox',
		) );
	...
	$wp_customize->add_section( 'deadline_front_page', array(
		...
		'choices'     => array(
			...
			'orange' => esc_html__( 'Orange', 'deadline' ),
			'pink' => esc_html__( 'Pink', 'deadline' ),
			'purple' => esc_html__( 'Purple', 'deadline' ),
			'red' => esc_html__( 'Red', 'deadline' ),
			'red2' => esc_html__( 'Red2', 'deadline' ),
...
function deadline_sanitize_widget_style( $input ) {
	$choices = array( 'black', 'blue', 'green', 'orange', 'pink', 'purple', 'red', 'red2', 'yellow' );
...
function deadline_page_background_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($page_background_color) ) );
}
...
function deadline_header_background_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($header_background_color) ) );
}
...
function deadline_footer_background_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($footer_background_color) ) );
}
...
function deadline_main_menu_background_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($menu_background_color) ) );
}
...
function deadline_highlight_background_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($highlight_background_color) ) );
}
...
function deadline_link_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($link_color) ) );
}
...
function deadline_main_text_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($main_text_color) ) );
}
...
function deadline_secondary_text_color_css() {
	...
	wp_add_inline_style( 'deadline-style', sprintf( $css, esc_attr($secondary_text_color) ) );
}
